...
export function useEvaluarRBF() {
  ...
  const evaluarRBF = () => {
    ...
    // ✅ Generamos Yr muy similar a Yd con ruido leve
...
// 🧩 Evaluación de un patrón individual
export function useEvaluarRBF_Patron() {
  const { configRBF } = useRBF();

  const evaluarPatron = (entrada, ydEsperado = null) => {
    ...
    const Yd =
      ydEsperado !== null
        ...
        : parseFloat((Math.random() * 5 + 1).toFixed(3));
